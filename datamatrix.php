...
$parameter=$excellfile.' "'.$excellsheet.'" '.$selected_group.' '.$tmpdir;
if($dataflag==1)
{
  $file = fopen($tmpdir."parameter.txt","w");
  fwrite($file,$parameter);
  fclose($file);
} else 
{
    $file = fopen($tmpdir."parameter.txt","r");
    $parameter=fgets($file);
    fclose($file);
}
...
exec('Rscript group_selection_v2.R '.$excellfile.' "'.$excellsheet.'" '.$selected_group.' '.$tmpdir);
//echo('Rscript group_selection_v2.R '.$excellfile.' "'.$excellsheet.'" '.$selected_group.' '.$tmpdir."\n");
exec('Rscript get_error_summary.R '.$tmpdir);
